...
export class AllocationRequestTable extends Kypo2Table<AllocationRequest> {
  ...
  constructor(resource: KypoPaginatedResource<AllocationRequest>, poolId: number, service: PoolAllocationRequestsPollingService, navigator: SandboxNavigator) {
    ...
    const rows = resource.elements.map(element => AllocationRequestTable.createRow(element, poolId, service, navigator));
    super(rows, columns);
    this.pagination = resource.pagination;
  }

  private static createRow(request: AllocationRequest, poolId: number, service: PoolAllocationRequestsPollingService, navigator: SandboxNavigator): Row<AllocationRequest> {
